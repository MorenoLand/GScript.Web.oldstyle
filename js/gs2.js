...
;(() => {
  ...
  const wasmUrl = new URL('gs2.wasm', scriptUrl || document.baseURI).href;
  let readyPromise = null;

  async function instantiateGoByte(go) {
  ...
  window.GoByteReady = function GoByteReady() {
    if (readyPromise) return readyPromise;
    readyPromise = (async () => {
      if (typeof Go !== 'function') {
        throw new Error('Go WASM runtime is missing.');
      }
      const go = new Go();
      const instance = await instantiateGoByte(go);
      go.run(instance);
      if (typeof window.GByteDecompileText !== 'function') {
        throw new Error('GoByte decompiler exports were not registered.');
      }
      return {
        decompileText: window.GByteDecompileText,
        decompileBytes: window.GByteDecompileBytes,
      };
    })();
    return readyPromise;
  };

  window.GS2Compiler = async function GS2Compiler() {
    await window.GoByteReady();
    throw new Error('GoByte is a decompiler only; compile mode is unavailable in this build.');
  };
})();
